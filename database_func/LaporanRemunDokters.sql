-- CREATE DEFINER=`rsflaporan`@`%` PROCEDURE `rsfPelaporan`.`LaporanRemunDokters`(
-- CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokters`(
-- CREATE PROCEDURE rsfPelaporan.LaporanRemunDokters(
DROP PROCEDURE IF EXISTS rsfPelaporan.LaporanRemunDokters;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokters`(
	IN `TGLAWAL` DATETIME, 
...
BEGIN
	/* --------------------------------------------------------------------------------------------------------------- */
	/* -- rsfPelaporan.LaporanRemunDokters															-- */
	/* --------------------------------------------------------------------------------------------------------------- */
	DECLARE vRUANGAN VARCHAR(11);
	SET vRUANGAN = CONCAT(RUANGAN,'%');
	SET @sqlText = CONCAT(
		"	
			SELECT		max(mPegawai.id) as dokter_id,
						max(mPegawai.nama) as dokter_nama,
				...
				group   by mPegawai.id
				order   by mPegawai.nama
				"
	);
	-- call rsfPelaporan.LaporanRemunDokters('2023-09-11','2023-09-11','0','0',864);
