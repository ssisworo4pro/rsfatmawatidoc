-- CREATE DEFINER=`rsflaporan`@`%` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVEksm`(
-- CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVEksm`(
-- CREATE PROCEDURE rsfPelaporan.LaporanRemunDokterEXECUTIVEksm(
DROP PROCEDURE IF EXISTS rsfPelaporan.LaporanRemunDokterEXECUTIVEksm;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVEksm`(
	IN `TGLAWAL` DATETIME, 
...
BEGIN
	/* --------------------------------------------------------------------------------------------------------------- */
	/* -- rsfPelaporan.LaporanRemunDokterEXECUTIVEksm															-- */
	/* --------------------------------------------------------------------------------------------------------------- */
	...
	SET @sqlText = CONCAT(
		"	
			select		dokter_nama,
						dokter_ksm,
						/* 
						-- sum(case kegiatan_kelompok when 'lab' then kegiatan_qty else 0 end) as qty_lab,
						-- sum(case kegiatan_kelompok when 'lab' then kegiatan_jasa else 0 end) as jasa_lab,
						-- sum(case kegiatan_kelompok when 'rad' then kegiatan_qty else 0 end) as qty_rad,
						-- sum(case kegiatan_kelompok when 'rad' then kegiatan_jasa else 0 end) as jasa_rad,
						*/
						FORMAT(sum(case kegiatan_kelompok when 'igh' then kegiatan_qty else 0 end),0) as qty_igh,
						FORMAT(sum(case kegiatan_kelompok when 'igh' then kegiatan_jasa else 0 end),0) as jasa_igh,
						FORMAT(sum(case kegiatan_kelompok when 'bedah prima' then kegiatan_qty else 0 end),0) as qty_bp,
						FORMAT(sum(case kegiatan_kelompok when 'bedah prima' then kegiatan_jasa else 0 end),0) as jasa_bp,
						FORMAT(sum(case kegiatan_kelompok when 'ukvi' then kegiatan_qty else 0 end),0) as qty_ukvi,
						FORMAT(sum(case kegiatan_kelompok when 'ukvi' then kegiatan_jasa else 0 end),0) as jasa_ukvi,
						FORMAT(sum(kegiatan_qty),0) as qty_total,
						FORMAT(sum(kegiatan_jasa),0) as jasa_total
						/* 
						-- kegiatan_jasa,
						-- kegiatan_qty,
						-- kegiatan_kelompok,
				...
				from	(
							SELECT		max(mPegawai.nama) as dokter_nama,
										...
										sum(tarif_griya.jasa_dr) as kegiatan_jasa,
										sum(1) as kegiatan_qty,
										max(
										case pj.JENIS
												...
												if(ifnull(mLokasiLayan.remun_klp,'') = 'penunjang', 'penunjang', 
													if(rsfMaster.getTrfTindNonKelasBHP(lynTind.TINDAKAN, lynTind.TANGGAL) > 0, 'tindakan', 
														if(ifnull(mLokasiLayan.remun_klp,'') = 'rawatinap', 'visite', 
															if(ifnull(mLokasiLayan.remun_klp,'') = 'rawatjalan', 'rawatjalan', 
																if(ifnull(mLokasiLayan.remun_klp,'') = 'executive', 'rawatjalan', 'penunjang')))))) 
								order   by	mPegawaiKsm.ksm_fatmawati, mPegawai.nama
						) sqKsm
				group   by 	dokter_ksm, dokter_nama
				order	by 	dokter_ksm, dokter_nama
				"
	);
	-- call rsfPelaporan.LaporanRemunDokterEXECUTIVEksm('2023-09-11','2023-09-11','0','0',864,1);
