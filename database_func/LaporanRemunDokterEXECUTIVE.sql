-- CREATE DEFINER=`rsflaporan`@`%` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVE`(
-- CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVE`(
-- CREATE PROCEDURE rsfPelaporan.LaporanRemunDokterEXECUTIVE(
DROP PROCEDURE IF EXISTS rsfPelaporan.LaporanRemunDokterEXECUTIVE;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanRemunDokterEXECUTIVE`(
	IN `TGLAWAL` DATETIME, 
...
BEGIN
	/* --------------------------------------------------------------------------------------------------------------- */
	/* -- rsfPelaporan.LaporanRemunDokterEXECUTIVE																			-- */
	/* --------------------------------------------------------------------------------------------------------------- */
	DECLARE vRUANGAN VARCHAR(11);
	SET vRUANGAN = CONCAT(RUANGAN,'%');
	SET @sqlText = CONCAT(
		"	SELECT		lynTind.id as id_transaksi, 
						lptm.id as id_transaksi_detail,
						lynTind.TANGGAL as tgl_transaksi,
						case LEFT(mRuangLayan.ID,5) 
							when '10107' then 
								case LEFT(mRuangLayan.ID,7)
									when '1010702' then lynTind.TANGGAL
									when '1010701' then lynTindHasilLAB.tgl_hasil
									else lynTind.TANGGAL --
								end
							when '10108' then lynTindHasilRAD.TANGGAL
							when '10109' then lynTind.TANGGAL
							when '10127' then lynTind.TANGGAL
							else lynTind.TANGGAL
						end as tgl_hasil,
						case LEFT(mRuangLayan.ID,5) 
							when '10107' then 
								case LEFT(mRuangLayan.ID,7)
									when '1010702' then mPegHslRAD.NIP
									when '1010701' then lynTindHasilLAB.kd_pelaksana
									else mPegHslRAD.NIP --
								end
							when '10108' then mPegHslRAD.NIP --
							else IFNULL(mPegPelaksana.NIP, mPegInput.NIP)
						end as dokter_kode,
						mPegawai.nip as dokter_nip,
						mPegawai.nama as dokter_nama,
						mPegawaiKsm.ksm_fatmawati as dokter_ksm,
						dftDaftar.NOMOR as daftar_nomor,
						pj.JENIS as daftar_crbyrid,
						mref2.DESKRIPSI as daftar_crbyr,
						tp.RUANGAN as daftar_ruangan,
						mRuangDaftar.DESKRIPSI as daftar_ruang,
						dftDaftar.NORM as pasien_nomor,
						mps.NAMA as pasien_nama,
						ifnull(mtin.ID, 0) as kegiatan_id,
						mtin.NAMA as kegiatan_nama,
						mRuangLayan.DESKRIPSI as kegiatan_ruang,
						FORMAT(rsfMaster.getTrfTindNonKelas(mtin.ID, lynTind.TANGGAL),0) as kegiatan_tarif_rs,
						FORMAT(tarif_griya.jasa_rs2,0) as kegiatan_tarif_rs2,
						FORMAT(tarif_griya.jasa_dr,0) as kegiatan_tarif_jpd,
						FORMAT(tarif_griya.total_tarif,0) as kegiatan_tarif_total,
						FORMAT(tarif_griya.total_tarif_executive,0) as kegiatan_tarif_final,
						case ifnull(lptm.JENIS, 0)
							when 2 then persentarif.persen_anes_nonjkn
							else persentarif.persen_opr_nonjkn
						end as rumus_porsi_dokter_dari_tarif,
						FORMAT(
						case ifnull(lptm.JENIS, 0)
							when 2 then persentarif.persen_anes_nonjkn
							else persentarif.persen_opr_nonjkn
						end * tarif_griya.total_tarif_executive / 100,0)
						as jasa_dokter_dari_porsi_tarif,
						FORMAT(tarif_griya.jasa_dr,0) as jasa_dokter_dari_jpd,
						case pj.JENIS
								...
								if(rsfMaster.getTrfTindNonKelasBHP(lynTind.TINDAKAN, lynTind.TANGGAL) > 0, 'tindakan', 
									if(ifnull(mLokasiLayan.remun_klp,'') = 'rawatinap', 'visite', 
										if(ifnull(mLokasiLayan.remun_klp,'') = 'rawatjalan', 'rawatjalan', 
											if(ifnull(mLokasiLayan.remun_klp,'') = 'executive', 'rawatjalan', 'penunjang')))))) as dto_kegiatan_klp,
						CURRENT_TIMESTAMP() as sysdate_get
				...
				order   by mRuangLayanInst.DESKRIPSI , IFNULL(mPegPelaksana.NAMA, mPegInput.NAMA)"
	);
	-- call rsfPelaporan.LaporanRemunDokterEXECUTIVE('2023-09-01','2023-09-01','0','0','0');
