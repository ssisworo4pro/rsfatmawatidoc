-- CREATE DEFINER=`rsflaporan`@`%` PROCEDURE `rsfPelaporan`.`LaporanKegiatanLaboratoriumPerRuang`(
-- CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanKegiatanLaboratoriumPerRuang`(
-- CREATE PROCEDURE rsfPelaporan.LaporanKegiatanLaboratoriumPerRuang(
DROP PROCEDURE IF EXISTS rsfPelaporan.LaporanKegiatanLaboratoriumPerRuang;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`LaporanKegiatanLaboratoriumPerRuang`(
	IN `TGLAWAL` DATETIME, 
...
BEGIN
	...
	SET @sqlText = CONCAT(
		"	SELECT		max(r.DESKRIPSI) as ruangan,
				...
				GROUP	BY	r.ID, tm.TINDAKAN
				ORDER   BY  r.DESKRIPSI, t.NAMA"
	);
	-- call rsfPelaporan.LaporanKegiatanLaboratoriumPerRuang('2023-09-01','2023-09-01','0','0','0');
