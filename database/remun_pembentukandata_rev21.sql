...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `rsfPelaporan`.`remun_pembentukandata`(
	IN `aRincianTagihan` integer,
	IN `aBulan` CHAR(6),
	IN `aTglMulai` CHAR(10),
	IN `aTglAkhir` CHAR(10)
)
BEGIN
	/* --------------------------------------------------------------------------------------------------------------- */
	...
	-- header
	select count(1), max(id) into vOounted, vID from tremun_data where tahunbulan = aBulan and proses_sts = 1;
	IF vOounted = 0 THEN 
	...
	IF vID > 0 THEN
		...
		delete from tremun_datarinci_dft2 where id_remunproses = vID;
		delete from tremun_datarinci_dft3 where id_remunproses = vID;
		delete from tremun_datarinci where id_remunproses = vID;
		
		-- pemeriksaan dasar
		/* New process
		SELECT		vID as id_remunproses, ltm.id, lptm.id,
					0 as sts_batal, 0 as sts_sama, 1 as sts_tertinggi,
		...
		*/
		...
		insert into tremun_datarinci ( 
					id_remunproses, id_transaksi, id_transaksi_dtl, 
					sts_batal, sts_sama, sts_tertinggi,
					tgl_transaksi, dokter_nip, dokter_nama, dokter_ksm,
					...
					kegiatan_petugas_sts, kegiatan_tarif_rs, kegiatan_tarif, kegiatan_tarif_lyn, kegiatan_tarif_bhp, kegiatan_tarif_dokter, kegiatan_tarif_griya, kegiatan_skor,
					...
					sts_sweeping1, sts_sweeping2, sts_sweeping3, qty_pelaksana, qty_anestesi)
		SELECT		vID as id_remunproses, ltm.id, lptm.id,
					0 as sts_batal, 0 as sts_sama, 1 as sts_tertinggi,
					ltm.TANGGAL as tgl_transaksi,
					IFNULL(mpeg.NIP, mpg.NIP) as dokter_nip,
					...
					CURRENT_TIMESTAMP() as sysdate_in,
					CURRENT_TIMESTAMP() as sysdate_updt,
					1 as sts_sweeping1, 1 as sts_sweeping2, 1 as sts_sweeping3, 1 as qty_pelaksana, 0 as qty_anestesi
		...
		insert into tremun_datarinci ( 
					...
					sts_hapus, sts_remun, sysdate_in, sysdate_updt,
					sts_sweeping1, sts_sweeping2, sts_sweeping3, qty_pelaksana, qty_anestesi )
		SELECT		vID as id_remunproses, ltm.id, ltm.id,
					0 as sts_batal, 0 as sts_sama, 1 as sts_tertinggi,
			...
			from 	lis_bridging.kinerja_dokter_lab kl
					...
					LEFT JOIN master.tindakan mtin ON mtin.ID = ltm.TINDAKAN
					-- LEFT JOIN rsfMaster.mremun_skor_smf mskor ON mskor.KODE = mtin.ID AND mskor.kode_smf = IFNULL(smf.ID,smf2.ID)
					LEFT JOIN rsfMaster.mremun_skor_smf mskor ON mskor.KODE = mtin.ID AND mskor.kode_smf = smf2.ID
					LEFT JOIN rsfMaster.mremun_tindakan mtindVisite ON mtin.ID = mtindVisite.ID
		...
		SELECT		vID as id_remunproses, ltm.id, ltm.id,
			...
			-- rekap pengunjung per dokter
			insert into tremun_datarinci_dft2 (
							id_remunproses, id_pegawai,		daftar,
							daftar_bpjs, 	daftar_nbpjs, 	daftar_nbpjse,
							nilai_bpjs, 	nilai_nbpjs, 	nilai_nbpjse
						)
			select		id_remunproses, id_pegawai, sum(1) as daftar,
						0 as daftar_bpjs, 
						0 as daftar_nbpjs, 
						0 as daftar_nbpjse,
						0 as nilai_bpjs, 
						...
						0 as nilai_nbpjse
				from	(
							select 		trx.id_remunproses as id_remunproses, 
										mpgw.id as id_pegawai, 
										trx.dto_kegiatan_klp as dto_kegiatan_klp, 
										trx.dto_kegiatan_crbyr as dto_kegiatan_crbyr, 
										trx.daftar_nomor as daftar_nomor
								from	tremun_datarinci trx
										...
										left outer join rsfPelaporan.tremun_kegiatan_exclude kegiatanExclude
											on kegiatanExclude.kegiatan_id 			= trx.kegiatan_id
								where	trx.id_remunproses 							= vID and
										mpgwNip.nip 								= trx.dokter_nip and
										ifnull(kegiatanExclude.kegiatan_dokter,1) 	= 1 and
										mpgw.id 									= mpgwNip.id_pegawai
										-- and trx.id                             			< 1824150
								group   by 	id_remunproses, dokter_nip, daftar_nomor
						) tremun_datarinci_grouping
				group   by id_remunproses, id_pegawai;

			-- rekap pengunjung per dokter per carabayar
			insert into tremun_datarinci_dft3 (
							id_remunproses, id_pegawai,
							daftar_bpjs, 	daftar_nbpjs, 	daftar_nbpjse,
							nilai_bpjs, 	nilai_nbpjs, 	nilai_nbpjse
						)
			select		id_remunproses, id_pegawai,
						sum(if(dto_kegiatan_crbyr = 'JKN',1,0)) as daftar_bpjs, 
						sum(if(dto_kegiatan_crbyr = 'REGULER',1,0)) as daftar_nbpjs, 
						sum(if(dto_kegiatan_crbyr = 'EXECUTIVE',1,0)) as daftar_nbpjse,
						0 as nilai_bpjs, 
				...
				from	(
							select 		trx.id_remunproses as id_remunproses, 
								...
								where	trx.id_remunproses 							= vID and
										...
										mpgw.id 									= mpgwNip.id_pegawai
										-- and trx.id                             			< 1824150
								group   by 	id_remunproses, dokter_nip, daftar_nomor, dto_kegiatan_crbyr
						) tremun_datarinci_grouping
				group   by id_remunproses, id_pegawai;

		-- error trx.id_remunproses
		-- update		tremun_datarinci_dft2 upd,
		--			tremun_datarinci_dft3 updreff
		--	set		upd.daftar_bpjs 	= updreff.daftar_bpjs,
		--			upd.daftar_nbpjs	= updreff.daftar_nbpjs,
		--			upd.daftar_nbpjse	= updreff.daftar_nbpjse
		--	where	upd.id_remunproses	= updreff.id_remunproses and
		--			upd.id_pegawai		= updreff.id_pegawai and
		--			trx.id_remunproses 	= vID;
		...
		update		rsfPelaporan.tremun_datarinci trx
					...
					on kegiatan_klp.kegiatan_id = trx.kegiatan_id
			set		trx.dto_kegiatan_klp =
					ifnull(kegiatan_klp.dto_kegiatan_klp,
						if(ifnull(miKunjungan.remun_klp,'') = 'penunjang', 'penunjang', 
							if(rsfMaster.getTrfTindBHP(trx.kegiatan_id, trx.kegiatan_trfkelas, trx.tgl_transaksi) > 0, 'tindakan', 
								if(ifnull(miKunjungan.remun_klp,'') = 'rawatinap', 'visite', 
									if(ifnull(miKunjungan.remun_klp,'') = 'rawatjalan', 'rawatjalan', 
										if(ifnull(miKunjungan.remun_klp,'') = 'executive', 'rawatjalan', 'penunjang')))))),
					trx.dto_kegiatan_crbyr =
					case trx.daftar_crbyrid
						when 2 then
							case ifnull(miKunjungan.remun_klp,'') 
								when 'executive' then 'EXECUTIVE'
								else 'JKN'
		...
		-- todo
		-- update tremun_datarinci.rumus_porsi_dokter_dari_tarif karena ada dokter yang belum masuk nip sehingga nggak dapet smf
		-- update tremun_datarinci_dft
		...
		update		rsfPelaporan.tremun_datarinci trx
			...
			set     rumus_pengali_tarif_baru = 
					case trx.dto_kegiatan_crbyr 
						when 'REGULER' then 100
					...
					kegiatan_tarif = 
					case trx.dto_kegiatan_crbyr 
						when 'REGULER' then kegiatan_tarif_lyn
						when 'EXECUTIVE' then kegiatan_tarif_lyn
						else kegiatan_tarif_rs
					end
			where	trx.id_remunproses = vID;
	END IF;
	COMMIT;
