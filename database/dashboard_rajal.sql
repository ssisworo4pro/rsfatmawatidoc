DROP PROCEDURE IF EXISTS rsfPelaporan.dashboard_rajal;
DELIMITER //
CREATE PROCEDURE rsfPelaporan.dashboard_rajal(
	aToken   	TEXT,
	aParam		TEXT
)
BEGIN
	...
	IF (vVAlid) THEN
		...
		ELSEIF (vMethod = "homeChart") THEN
			...
			SET vTanggalAk = DATE_ADD(CURRENT_DATE(), INTERVAL 0 DAY);
			select		dataChart,
						Concat('Grafik Pendaftaran Pasien Rawat Jalan','') as judul1,
			...
			select		*
				from	(
											SELECT		@counter := @counter+1 AS row_counter,
														...
														srp.DOKTER,
														INST.NAMAINST, INST.ALAMATINST,
														Concat('Daftar Pasien ',instalasi.deskripsi) as judul1,
														Concat('Tanggal ',DATE_FORMAT(vTanggalAw,'%d-%m-%Y')) as judul2,
														'rajal' as parm0,
														'token' as parm1,
														'pendaftaran' as tautan1,
