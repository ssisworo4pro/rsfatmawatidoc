DROP PROCEDURE IF EXISTS rsfPelaporan.dashboard_dirut_data;
DELIMITER //
CREATE PROCEDURE rsfPelaporan.dashboard_dirut_data(
	aOBJ VARCHAR(99)
)
BEGIN
	START TRANSACTION;
		IF (aOBJ = "pasien28hari") THEN
			-- BENTUK DATA dashboardes_pasien28hari tahun 2022 dari SIMGOS mulai april 2022 hingga desember 2022
			delete from		rsfPelaporan.dashboardes_pasien28hari;
			insert into		rsfPelaporan.dashboardes_pasien28hari 
							(	pasien_norm,				kunjungan_instalasi_kd,		kunjungan_kelas_nm,
								kunjungan_carabayar_klp,	kunjungan_tanggal,			kunjungan_selesai,
								kunjungan_carabayar_nm,		kunjungan_ruangan_id,		kunjungan_nomor,
								kunjungan_instalasi_nm,		pendaftaran_nomor,			pendaftaran_tanggal,
								pasien_tgllahir,			pasien_nama
							)
				select 		daftar.NORM as pasien_norm,
							SUBSTR(kunj.RUANGAN,1,5) as kunjungan_instalasi_kd,
					...
					from 	master.pasien pasien,
							...
							on kunj.RUANG_KAMAR_TIDUR = rkt.ID
							left outer join
							master.ruang_kamar rk
							on rkt.RUANG_KAMAR = rk.ID
							left outer join
							(
								select		*
					...
					where 	daftar.NORM 	 = pasien.NORM and
							...
							kunj.MASUK      >= '2022-04-01' and 
							kunj.MASUK       < '2023-01-01';
			SELECT 		0 as statcode,
						count(1) as rowcount,
						concat('rsfPelaporan ', aOBJ, ', insert complete. now ', count(1) ,' row counted. ') as statmessage,
						'success' as data
				FROM	rsfPelaporan.dashboardes_pasien28hari;
		ELSEIF (aOBJ = "pasienNicuPicu") THEN
			delete from		rsfPelaporan.dashboardes_pasiennicupicu;
			insert into		rsfPelaporan.dashboardes_pasiennicupicu 
							(	pasien_norm,				kunjungan_instalasi_kd,		kunjungan_kelas_nm,
								kunjungan_carabayar_klp,	kunjungan_tanggal,			kunjungan_selesai,
								kunjungan_carabayar_nm,		kunjungan_ruangan_id,		kunjungan_nomor,
								kunjungan_instalasi_nm,		pendaftaran_nomor,			pendaftaran_tanggal,
			...
			SELECT 		0 as statcode,
						...
						concat('rsfPelaporan ', aOBJ, ', insert complete. now ', count(1) ,' row counted. ') as statmessage,
						'success' as data
				FROM	rsfPelaporan.dashboardes_pasiennicupicu;
		ELSEIF (aOBJ = "pasienDiagnosa") THEN
			delete from		rsfPelaporan.dashboardes_pasiendiagnosa;
			insert into		rsfPelaporan.dashboardes_pasiendiagnosa 
							(	pasien_norm,				kunjungan_instalasi_kd,		kunjungan_kelas_nm,
								kunjungan_carabayar_klp,	kunjungan_tanggal,			kunjungan_selesai,
								kunjungan_carabayar_nm,		kunjungan_ruangan_id,		kunjungan_nomor,
								kunjungan_instalasi_nm,		pendaftaran_nomor,			pendaftaran_tanggal,
								pasien_tgllahir,			pasien_nama,
								pasien_jeniskelamin, 		kunjungan_tanggal_keluar,	
								ksm_id, 					ksm_nm, 					ksm_dpjp, 
								icd10_kd,					icd10_nm,					icd10_nm_header
							)
				SELECT		pasien.NORM as pasien_norm, 
							...
							DATE_FORMAT(kunj_b.KELUAR,'%Y/%m/%d %H:%i:%s') as kunjungan_tanggal_keluar,
							smf.ID as ksm_id, 
			...
			SELECT 		0 as statcode,
						...
						concat('rsfPelaporan ', aOBJ, ', insert complete. now ', count(1) ,' row counted. ') as statmessage,
						'success' as data
				FROM	rsfPelaporan.dashboardes_pasiendiagnosa;
		ELSE
			SELECT 		20001 as statcode,
						0 as rowcount,
						concat('rsfPelaporan, object ''', aOBJ,''' tidak ditemukan.') as statmessage,
						'' as data;
		END IF;
	COMMIT;
END //
