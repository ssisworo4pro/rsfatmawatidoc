select		max(depo_nama) as depo_nama,
			max(katalog_kode) as katalog_kode,
			max(katalog_nama) as katalog_nama,
			sum(qty_opname) as qty_opname,
			sum(qty_beli) as qty_beli,
			sum(qty_mmasuk) as qty_mmasuk,
			sum(qty_mkeluar) as qty_mkeluar,
			sum(qty_prod) as qty_prod,
			sum(qty_resep) as qty_resep,
			sum(qty_resep_retur) as qty_resep_retur,
			sum(qty_jual) as qty_jual,
			sum(qty_jual_retur) as qty_jual_retur,
			sum(qty_akhir) as qty_akhir
	from	(
				SELECT		max(laporanSOdepoREKON.depo_nama) as depo_nama,
							max(subsubquery.katalog_id) as katalog_id,
							max(barang.KODE_BARANG) as katalog_kode_old,
					...
					from	(
							...
							(
								select		case depo_nama 	when 'anggrek' then '101030106'
											...
											depo_nama,
											katalog_id,
											COALESCE(koreksi_kode, katalog_kode) as katalog_kode
									from	laporan_so_depo
							) laporanSOdepoREKON
					WHERE	laporanSOdepoREKON.ruangan_kode = subsubquery.ruangan_kode and
							laporanSOdepoREKON.katalog_id = subsubquery.katalog_id and
							(
					...
					ORDER 	by subsubquery.ruangan_kode, subsubquery.katalog_kode
			) superQuery
	GROUP   by depo_nama, katalog_kode
	ORDER 	by depo_nama, katalog_kode
