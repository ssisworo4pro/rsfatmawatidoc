insert into rsfPelaporan.laporan_so_akhirtahun
	(	depo_kode, katalog_kode, depo_nama,
		katalog_nama, katalog_satuan, katalog_merk,
		opname, beli, mmasuk, mkeluar, prod, resep, resep_retur,
		jual, jual_retur, tambil, akhir,
		katalog_kode_asal, katalog_kode_koreksi, katalog_kode_grp,
		koreksi_kode2, koreksi_keterangan )
	SELECT		max(tableTransaksi.ruangan_kode) as depo_kode,
				max(laporanSOdepoREKON.katalog_kode) as katalog_kode,
				max(tableTransaksi.depo_nama) as depo_nama,
				max(barang.NAMA) as katalog_nama,
				max(msatuan.NAMA) as katalog_satuan,
				max(mreff.DESKRIPSI) as katalog_merk,
				sum(tableTransaksi.qty_opname) as opname,
				sum(tableTransaksi.qty_beli) as beli,
				sum(tableTransaksi.qty_mmasuk) as mmasuk,
				sum(tableTransaksi.qty_mkeluar) as mkeluar,
				sum(tableTransaksi.qty_prod) as prod,
				sum(tableTransaksi.qty_resep) as resep,
				sum(tableTransaksi.qty_resep_retur) as resep_retur,
				sum(tableTransaksi.qty_jual) as jual,
				sum(tableTransaksi.qty_jual_retur) as jual_retur,
				sum(tableTransaksi.qty_tambil) as tambil,
				sum(tableTransaksi.qty_akhir) as akhir,
				...
				max(laporanSOdepoREKON.koreksi_kode2) as koreksi_kode2,
		...
		FROM	(
				...
				(
					select		case depo_nama 	when 'anggrek' then '101030106'
								...
								koreksi_kode2 as koreksi_kode2,
		...
		GROUP   BY  laporanSOdepoREKON.depo_nama, laporanKatalogGrp.katalog_kode_grp
