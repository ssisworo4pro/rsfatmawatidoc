insert into rsfPelaporan.laporan_so_trx
	(	depo_kode, katalog_id, depo_nama, katalog_kode, 
		katalog_nama, katalog_satuan, 
		opname, beli, mmasuk, mkeluar, prod, resep, resep_retur,
		jual, jual_retur, tambil, akhir )
SELECT		max(laporanSOdepoLIST.ruangan_kode) as depo_kode,
			max(subsubquery.katalog_id) as katalog_id,
			max(laporanSOdepoLIST.depo_nama) as depo_nama,
			...
			sum(subsubquery.qty_opname) as opname,
			sum(subsubquery.qty_beli) as beli,
			sum(subsubquery.qty_masuk) as mmasuk,
			sum(subsubquery.qty_keluar) as mkeluar,
			sum(subsubquery.qty_prod) as prod,
			sum(subsubquery.qty_trx) as resep,
			sum(subsubquery.qty_trx_retur) as resep_retur,
			sum(subsubquery.qty_jual) as jual,
			sum(subsubquery.qty_jual_retur) as jual_retur,
			sum(subsubquery.qty_tambil) as tambil,
			...
			+ sum(subsubquery.qty_jual_retur) as akhir
	from	(
				select		convert(
