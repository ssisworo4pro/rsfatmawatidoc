select 		jurnal.coa_perusahaan			as coa_perusahaan,
			jurnal.no_dokumen				as coa_dokumen,
			jurnal.nilai_sisa               as coa_saldo,
	...
	from 	rsfPelaporan.tjurnal_hutangobat jurnal
			left outer join
			rsfTeamterima.transaksif_penerimaan teamTrmx
			on 	teamTrmx.no_doc 			= jurnal.no_dokumen and
				teamTrmx.ver_gudang 		= 1
			left outer join
			rsfTeamterima.tdetailf_penerimaan teamTrmxDtl
			on teamTrmx.kode 				 = teamTrmxDtl.kode_reff
			left outer join
			...
			on teamMasterK.kode 			 = teamTrmxDtl.id_reffkatalog
			left outer join 
			(
	...
	order	by jurnal.coa_perusahaan, jurnal.no_dokumen
...
select 		jurnal.coa_perusahaan			as coa_perusahaan,
		    ...
		    masterBarang.KODE_PERSEDIAAN	as sub_kelompok,
			teamMasterK.nama_barang			as nama_barang,
			returrekanan.jumlah_item 		as retur_jumlah_item,
			teamTrmxDtl.jumlah_item			as terima_jumlah_item,
			teamTrmxDtl.hp_item								as hp_item,
	...
	from 	rsfPelaporan.tjurnal_hutangobat jurnal
			left outer join
			rsfTeamterima.transaksif_penerimaan teamTrmx
			on jurnal.no_dokumen = teamTrmx.no_doc
			left outer join
			(
			...
			on
			teamTrmx.kode 				= returrekanan.kode_refftrm
			left outer join
			rsfTeamterima.tdetailf_penerimaan teamTrmxDtl
			on teamTrmx.kode 				 = teamTrmxDtl.kode_reff
			left outer join
